...
# --- Funding column ---
funding_col <- training_meta$funding_col_name
...
server <- function(input, output, session) {
  ...
  observeEvent(input$go_pred, {
    ...
    for(cat in training_meta$categorical){
      new_df[[cat]] <- factor(new_df[[cat]], levels = training_meta$levels[[cat]])
    }
    ...
    # Predict safely
    pred_log <- tryCatch({
      predict(rf_model, newdata = new_df)
    }, error = function(e) {
    ...
    if(!is.na(pred_log)){
      ...
      output$prediction <- renderText("Prediction failed")
    }
  })
